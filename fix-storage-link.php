...
if (is_dir(__DIR__ . '/public')) {
    $webRootDir = __DIR__ . '/public';
} elseif (is_dir(__DIR__ . '/public_html')) {
    $webRootDir = __DIR__ . '/public_html';
} else {
    echo "<div class='error'><strong>Error:</strong> Could not find public or public_html directory.</div>";
    exit;
}

$publicStorage = $webRootDir . '/storage';
$storagePublic = __DIR__ . '/storage/app/public';

echo "<div class='info'>";
echo "<strong>Project Root:</strong> " . __DIR__ . "<br>";
echo "<strong>Detected Web Root:</strong> " . basename($webRootDir) . "<br>";
echo "<strong>Target Storage Path:</strong> storage/app/public<br>";
echo "<strong>Public Link Path:</strong> " . basename($webRootDir) . "/storage";
echo "</div>";

// Check if storage/app/public exists
if (!file_exists($storagePublic)) {
    echo "<div class='warning'>";
    echo "Warning: storage/app/public directory does not exist!<br>";
...
if (is_link($publicStorage)) {
    ...
    echo "<div class='warning'>";
    echo "Warning: " . basename($webRootDir) . "/storage is a real DIRECTORY, not a symlink!<br>";
    echo "This will be backed up before creating the proper symlink.";
    echo "</div>";
    
    if (is_dir($publicStorage)) {
        // Create backup
        $backupDir = $webRootDir . '/storage_backup_' . date('Y-m-d_His');
...
if ($success && is_link($publicStorage)) {
    ...
    echo "cd " . __DIR__ . "\n";
    echo "cd " . __DIR__ . "\n";
    echo "rm -rf " . basename($webRootDir) . "/storage\n";
    echo "ln -s ../storage/app/public " . basename($webRootDir) . "/storage\n";
