...
<div class="row mb-4">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <small class="text-muted">Quick overview of your assigned sections</small>
        </div>
        <div class="row">
            @php
                // Calculate section counts dynamically from the actual student data
                                ...
                                <div class="student-image-section">
                                    @if($student->picture)
                                        @php
                                            $imagePath = null;
                                            $possiblePaths = [
                                                'storage/student_pictures/' . $student->picture,
                                                'storage/' . $student->picture,
                                                'student_pictures/' . $student->picture
                                            ];
                                            ...
                                            if(!$imagePath && Storage::disk('public')->exists('student_pictures/' . $student->picture)) {
                                                $imagePath = Storage::url('student_pictures/' . $student->picture);
                                            }
                                        @endphp
                                        
                                        @if($imagePath)
                                            <img src="{{ $imagePath }}" alt="Student Picture" 
                                                 class="student-photo" 
                    ...
                    <div class="col-md-4 text-center">
                        @if($student->picture)
                            @php
                                ...
                                foreach($possiblePaths as $path) {
                                    if(file_exists(public_path($path))) {
                                        $modalImagePath = asset($path);
                                        break;
                                    }
                                }
                                
                                if(!$modalImagePath && Storage::disk('public')->exists('student_pictures/' . $student->picture)) {
                                    $modalImagePath = Storage::url('student_pictures/' . $student->picture);
                                }
                            @endphp
                            
                            @if($modalImagePath)
                                <img src="{{ $modalImagePath }}" alt="Student Picture" 
                                     class="img-fluid rounded-circle mb-3" 
                                     style="width: 150px; height: 150px; object-fit: cover; border: 3px solid #dee2e6;"
                                     onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="bg-light rounded-circle mx-auto mb-3 d-flex align-items-center justify-content-center" 
                                     style="width: 150px; height: 150px; border: 3px solid #dee2e6; display: none;">
