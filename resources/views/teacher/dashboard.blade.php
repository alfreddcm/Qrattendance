...
@php
    ...
    if($currentSchoolYear) {
        $startDate = \Carbon\Carbon::parse($currentSchoolYear->start_date);
        $endDate = \Carbon\Carbon::parse($currentSchoolYear->end_date);
    ...
    $displayData = [
        'semester_name' => $isActiveSchoolYear ? $currentSchoolYear->name : 'No Active School Year',
        'school_name' => auth()->user()->school ? auth()->user()->school->name : 'N/A',
        'school_year' => $isActiveSchoolYear ? \Carbon\Carbon::parse($currentSchoolYear->start_date)->format('Y') . ' - ' . \Carbon\Carbon::parse($currentSchoolYear->end_date)->format('Y') : 'N/A',
        'date_range' => $isActiveSchoolYear ? \Carbon\Carbon::parse($currentSchoolYear->start_date)->format('M j, Y') . ' – ' . \Carbon\Carbon::parse($currentSchoolYear->end_date)->format('M j, Y') : 'Not Available',
        ...
        'student_count' => $isActiveSchoolYear ? ($studentCount ?? 0) : 0,
        'present_count' => $isActiveSchoolYear ? ($presentCount ?? 0) : 0,
        'absent_count' => $isActiveSchoolYear ? ($absentCount ?? 0) : 0,
        'incomplete_profiles_count' => $isActiveSchoolYear ? (isset($studentsWithMissingInfo) ? $studentsWithMissingInfo->count() : 0) : 0,
        'most_punctual' => $isActiveSchoolYear ? ($mostPunctual ?? null) : null,
        'most_absent' => $isActiveSchoolYear ? ($mostAbsent ?? null) : null,
        'students_with_missing_info' => $isActiveSchoolYear ? ($studentsWithMissingInfo ?? collect()) : collect(),
    ];
@endphp
...
<div class="compact-layout">
    ...
    <div class="container-fluid">
        ...
        @if($isActiveSchoolYear && $displayData['students_with_missing_info']->count() > 0)
        <div class="row mb-3">
            <div class="col-12">
        ...
        @elseif(!$isActiveSchoolYear && $schoolYears->count() > 0)
        <div class="row mb-3">
            <div class="col-12">
                <div class="card shadow-sm border-warning card-compact">
                    <div class="card-body text-center py-4">
                        <i class="fas fa-calendar-times fa-2x text-warning mb-2"></i>
                        <h6 class="text-warning mb-2">No Active School Year</h6>
                        <p class="text-muted mb-3 fs-8">
                            The current date ({{ $today->format('M j, Y') }}) is outside any school year date range.
                        </p>
                        <a href="{{ route('teacher.school-years') }}" class="btn btn-warning btn-compact">
                            <i class="fas fa-plus me-1"></i>View School Years
                        </a>
                    </div>
                </div>
            </div>
